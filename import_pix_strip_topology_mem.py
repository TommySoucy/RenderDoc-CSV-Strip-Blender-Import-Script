...
class PIX_CSV_Operator(bpy.types.Operator):
    ...
    bl_label = "Import PIX csv Strip Mem"
...
def importCSV(filepath=None, mirror_x=False, vertex_order=True, global_matrix=None):
    ...
    with open(filepath) as f:
        ...
        ################################## fill vertices, triangles (faces), normals, and uvs arrays ###########################
            
        #this script puts everything in memory so we can seek specific previous vertices whenever needed
        #store all vertices in memory
        verts=[]
        ...
        ###############################
        # this is where ends my work, the rest of this script isn't mine
...
def menu_func_import(self, context):
    self.layout.operator(PIX_CSV_Operator.bl_idname, text="PIX CSV Strip Mem (.csv)")
